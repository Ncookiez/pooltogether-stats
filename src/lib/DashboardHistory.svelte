<script lang="ts">
	...
	let selectedDraw: number = 0;
	let depositFilter: number = 0;

	// Reactive Loading Checks:
	$: latestDepositsLoaded = chains.every(chain => $loading[chain].deposits === 'done');
	$: latestDelegationsLoaded = chains.every(chain => $loading[chain].delegations === 'done');
	$: latestDepositsErrored = !chains.every(chain => $loading[chain].deposits !== 'failed');
	$: latestDelegationsErrored = !chains.every(chain => $loading[chain].delegations !== 'failed');
