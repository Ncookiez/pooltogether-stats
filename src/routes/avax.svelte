<script lang="ts">
	...
	$: winlessWithdrawals = $avaxData.winlessWithdrawals ? $avaxData.winlessWithdrawals.filter(wallet => wallet.maxBalance > (minWinlessWithdrawalsBalance ?? 0)) : [];
	...
	const setChartData = (startTime: number, endTime: number) => {
		if($avaxData.tvlOverTime && $avaxData.depositsOverTime && $avaxData.withdrawalsOverTime && $avaxData.claimsOverTime && $avaxData.delegationsOverTime && $avaxData.yieldOverTime && $avaxData.tvlDistribution && $ethData.movingUsers && $polyData.movingUsers && $opData.movingUsers) {
			...
			} else if(startTime !== 0 && $avaxData.maxTimestamp) {
				timeFilters = { start: startTime, end: $avaxData.maxTimestamp };
				customTimestamps = getRangeArray(startTime, $avaxData.maxTimestamp, ticks);
			} else if(endTime !== defaultMaxTimestamp && $avaxData.minTimestamp) {
				timeFilters = { start: $avaxData.minTimestamp, end: endTime };
				customTimestamps = getRangeArray($avaxData.minTimestamp, endTime, ticks);
			}

			// Setting Raw Data:
			const depositsOverTime = customTimestamps ? getDepositsOverTime($avaxData, ticks, customTimestamps) : $avaxData.depositsOverTime;
			const withdrawalsOverTime = customTimestamps ? getWithdrawalsOverTime($avaxData, ticks, customTimestamps) : $avaxData.withdrawalsOverTime;
			const claimsOverTime = customTimestamps ? getClaimsOverTime($avaxData, ticks, customTimestamps) : $avaxData.claimsOverTime;
			const delegationsOverTime = customTimestamps ? getDelegationsOverTime($avaxData, ticks, customTimestamps) : $avaxData.delegationsOverTime;
			const tvlOverTime = customTimestamps ? getTVLOverTime(depositsOverTime, withdrawalsOverTime, claimsOverTime) : $avaxData.tvlOverTime;
			const yieldOverTime = customTimestamps ? getYieldOverTime($avaxData, ticks, customTimestamps) : $avaxData.yieldOverTime;
			const tvlDistribution = $avaxData.tvlDistribution;
			...
			const movingUsersChartData: number[] = [
				ethMovingUsers.movedToAVAX.amount,
				polyMovingUsers.movedToAVAX.amount,
				opMovingUsers.movedToAVAX.amount
			];
...
<Highlight hide={$endTimestamp !== defaultMaxTimestamp}>
	...
	<span class="big highlight">{$avaxData.balances.data.filter(entry => entry.balance > 0).length.toLocaleString(undefined)}+ Depositors</span>
