<script lang="ts">
	...
	$: winlessWithdrawals = $polyData.winlessWithdrawals ? $polyData.winlessWithdrawals.filter(wallet => wallet.maxBalance > (minWinlessWithdrawalsBalance ?? 0)) : [];
	...
	const setChartData = (startTime: number, endTime: number) => {
		if($polyData.tvlOverTime && $polyData.depositsOverTime && $polyData.withdrawalsOverTime && $polyData.claimsOverTime && $polyData.delegationsOverTime && $polyData.yieldOverTime && $polyData.tvlDistribution && $ethData.movingUsers && $avaxData.movingUsers && $opData.movingUsers) {
			...
			} else if(startTime !== 0 && $polyData.maxTimestamp) {
				timeFilters = { start: startTime, end: $polyData.maxTimestamp };
				customTimestamps = getRangeArray(startTime, $polyData.maxTimestamp, ticks);
			} else if(endTime !== defaultMaxTimestamp && $polyData.minTimestamp) {
				timeFilters = { start: $polyData.minTimestamp, end: endTime };
				customTimestamps = getRangeArray($polyData.minTimestamp, endTime, ticks);
			}

			// Setting Raw Data:
			const depositsOverTime = customTimestamps ? getDepositsOverTime($polyData, ticks, customTimestamps) : $polyData.depositsOverTime;
			const withdrawalsOverTime = customTimestamps ? getWithdrawalsOverTime($polyData, ticks, customTimestamps) : $polyData.withdrawalsOverTime;
			const claimsOverTime = customTimestamps ? getClaimsOverTime($polyData, ticks, customTimestamps) : $polyData.claimsOverTime;
			const delegationsOverTime = customTimestamps ? getDelegationsOverTime($polyData, ticks, customTimestamps) : $polyData.delegationsOverTime;
			const tvlOverTime = customTimestamps ? getTVLOverTime(depositsOverTime, withdrawalsOverTime, claimsOverTime) : $polyData.tvlOverTime;
			const yieldOverTime = customTimestamps ? getYieldOverTime($polyData, ticks, customTimestamps) : $polyData.yieldOverTime;
			const tvlDistribution = $polyData.tvlDistribution;
			const ethMovingUsers = customTimestamps ? getMovingUsers($ethData.withdrawals.data, $ethData.deposits.data, $polyData.deposits.data, $avaxData.deposits.data, $opData.deposits.data, timeFilters) : $ethData.movingUsers;
			...
			const movingUsersChartData: number[] = [
				ethMovingUsers.movedToPOLY.amount,
				avaxMovingUsers.movedToPOLY.amount,
				opMovingUsers.movedToPOLY.amount
			];
...
<Highlight hide={$endTimestamp !== defaultMaxTimestamp}>
	...
	<span class="big highlight">{$polyData.balances.data.filter(entry => entry.balance > 0).length.toLocaleString(undefined)}+ Depositors</span>
